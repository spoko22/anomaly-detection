class FrequencyIndicator:
    ...
    def using_mean(self, dataset, column, new_column=None, remove_old=False):
        if column not in self.median_cols:
            self.median_cols[column] = self.reference_set[column].value_counts()
    ...
    def absolute(self, dataset, column, new_column=None, remove_old=False):
        ...
        if new_column is None:
            new[column] = new[column].apply(lambda x: vals[x]/total if x in vals else 0)
        else:
            new[new_column] = new[column].apply(lambda x: vals[x]/total if x in vals else 0)
            if remove_old:
                return new.drop(column, axis=1)
        return new